...
export default function AdminDashboard({
  ...
  const handleSimulateApiCall = async (type: 'stepfun' | 'tts' | 'clone' | 'other') => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const typeNames: Record<string, string> = {
          stepfun: "StepFun step-3.7-flash 文本生成",
          tts: "Cosmic TTS 语音合成",
          clone: "声音克隆算法解析",
          other: "运营中心业务逻辑"
        };
        ...
        <div className="max-w-md w-full space-y-8 bg-white p-8 rounded-3xl border border-zinc-200 shadow-sm relative">
          ...
          <div>
            ...
            <p className="mt-2 text-center text-xs text-zinc-400">
              {isRegisterMode ? "后台管理系统的管理员自主管理与分权机制" : "请输入耳畔童话系统后台管理员凭证以继续"}
            </p>
          </div>
    ...
    <div className="w-full min-h-[640px] bg-zinc-50 text-zinc-900 rounded-3xl overflow-hidden border border-zinc-200 flex flex-col font-sans relative shadow-sm">
      ...
      <header className="bg-zinc-900 text-white px-6 py-4 border-b border-zinc-950 flex justify-between items-center select-none shrink-0 shadow-xs">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h1 className="text-sm font-bold tracking-tight">耳畔童话 · 后台业务中台运营系统</h1>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-3 text-xs pt-1">
                      ...
                      <div>
                        ...
                        <span className="font-bold block text-sm mt-0.5 text-zinc-800">约 3.2 秒 (StepFun 主模型)</span>
                      </div>
                    </div>
                  </div>

                  <div className="bg-zinc-50 p-3.5 rounded-xl border border-zinc-200 text-[10px] text-zinc-500 leading-relaxed">
                    🌟 耳畔建议：当前睡前故事高频生成，可上架更多【安睡森林】相关的插图大纲 and 模板！
                  </div>
                </div>
          ...
          {activeTab === "api" && (() => {
            ...
            const filteredLogs = rawLogs.filter((log: any) => {
              const serviceMatch = 
                ...
                (apiServiceFilter === "stepfun" && log.service?.toLowerCase().includes("stepfun")) ||
                ...
                (apiServiceFilter === "other" && !log.service?.toLowerCase().includes("stepfun") && !log.service?.toLowerCase().includes("tts") && !log.service?.toLowerCase().includes("synthesizer") && !log.service?.toLowerCase().includes("clone"));
            ...
            return (
              <div className="space-y-6 animate-fade-in">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-zinc-400 mt-1">
                        实时监控本系统发起的 StepFun（阶跃星辰）大模型文本生成调用、声音克隆算法计算，以及 Cosmic TTS 有声小说声音转码。
                      </p>
                    </div>
                  </div>
                ...
                <div className="bg-zinc-900 text-white p-5 rounded-2xl border border-zinc-950 space-y-3.5 shadow-sm">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <button
                      ...
                      onClick={() => handleSimulateApiCall('stepfun')}
                      className="bg-emerald-600 hover:bg-emerald-500 border border-emerald-500 disabled:opacity-50 text-white text-xs px-3.5 py-2.5 rounded-xl font-bold transition flex items-center justify-center gap-2 cursor-pointer"
                    >
                      <Play className="w-3 h-3 text-white fill-white" />
                      <span>模拟 StepFun 文本生成</span>
                    </button>
                ...
                <div className="grid grid-cols-5 gap-4">
                  ...
                  <div className="bg-white p-4.5 rounded-2xl border border-zinc-200 shadow-xs flex flex-col justify-between">
                    <div>
                      <span className="text-[10px] text-zinc-400 block font-bold uppercase tracking-wider font-mono">StepFun Text Calls</span>
                      <span className="text-2xl font-black text-emerald-600 block mt-1">{apiStats.stepfunTextCalls} 次</span>
                    </div>
                                ...
                                <tr className="border-b border-zinc-100 hover:bg-zinc-50/50 text-zinc-700 transition">
                                  ...
                                  <td className="p-4 text-zinc-600 font-sans text-xs">
                                    <span className="flex items-center gap-1.5">
                                      <span className={`w-1.5 h-1.5 rounded-full ${log.service?.includes("StepFun") ? "bg-emerald-500" : log.service?.includes("TTS") ? "bg-purple-500" : "bg-blue-400"}`}></span>
                                      {log.service}
                                    </span>
                                  </td>
...
function SafetyControlTab({ db, refreshDb, triggerAlert }: SafetyControlTabProps) {
  ...
  const handleSaveSafetyConfig = async () => {
    ...
    try {
      ...
      if (res.ok) {
        refreshDb();
        triggerAlert("🎉 耳畔内容安全防火墙配置已实时更新并发布应用！");
      } else {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl border border-zinc-200 p-5 shadow-xs flex flex-col h-[610px] overflow-hidden">
          <div className="flex items-center gap-2 border-b border-zinc-100 pb-2.5 shrink-0">
            ...
            <h4 className="text-xs font-extrabold text-zinc-800">3. 耳畔安全守护：有害/违规内容拦截与复核流水</h4>
          </div>

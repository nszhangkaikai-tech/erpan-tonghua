// 故事绘本模板与提示词构造（从 backend/src/storybookTemplates.ts 原样搬运，供云函数调用）
// 纯逻辑模块，无外部依赖。
...
const CURATED = [
  { id: 'tpl_bedtime_family', name: '月光下的晚安抱抱', cover: 'https://images.unsplash.com/photo-1518156677180-95a2893f3e9f?w=500&q=80', ageGroup: '2-5岁', theme: '睡前安抚', educationalGoal: '情绪放松', scene: '温馨家庭', mainCharacter: { name: '团团', role: '需要晚安陪伴的小熊', personality: '温柔敏感，喜欢听故事' }, duration: 'short', description: '把睡前的小小不安变成一个温暖的晚安仪式，让孩子在熟悉的家中安心入睡。', isRecommended: true, useCount: 0, visualStyle: { ...DEFAULT_STYLE, palette: '月光蓝、奶油白、柔和薰衣草紫', lighting: '窗边月光与床头暖灯交织的柔和夜景' }, coverPromptSeed: COVER_SEED, contentPromptSeed: CONTENT_SEED, chapterBeats: ['准备晚安仪式', '发现并表达不安', '在陪伴中放松入睡'] },
  { id: 'tpl_courage_castle', name: '城堡里的第一步', cover: 'https://images.unsplash.com/photo-1518156677180-95a2893f3e9f?w=500&q=80', ageGroup: '4-7岁', theme: '勇敢自信', educationalGoal: '面对困难不退缩', scene: '魔法城堡', mainCharacter: { name: '勇勇', role: '第一次参加任务的小骑士', personality: '认真善良，偶尔会紧张' }, duration: 'medium', description: '小骑士在朋友的鼓励下迈出第一步，理解勇敢不是不害怕，而是愿意试一试。', isRecommended: true, useCount: 0, visualStyle: { ...DEFAULT_STYLE, palette: '天空蓝、暖橙、彩虹黄', lighting: '明亮的晨光和魔法星尘' }, coverPromptSeed: COVER_SEED, contentPromptSeed: CONTENT_SEED, chapterBeats: ['接受小任务', '遇到需要尝试的难题', '用自己的方法完成挑战', '把勇气分享给朋友'] },
  { id: 'tpl_friendship_undersea', name: '海底王国的分享日', cover: 'https://images.unsplash.com/photo-1518156677180-95a2893f3e9f?w=500&q=80', ageGroup: '3-6岁', theme: '友情人际', educationalGoal: '学会分享', scene: '海底世界', mainCharacter: { name: '泡泡', role: '喜欢收集贝壳的小海马', personality: '聪明可爱，正在学习倾听' }, duration: 'medium', description: '一场海底分享日让小海马学会倾听伙伴、表达需要，并和朋友一起找到更好的办法。', isRecommended: true, useCount: 0, visualStyle: { ...DEFAULT_STYLE, palette: '草木绿、珊瑚橙、晴天蓝', lighting: '清透的海水光束和温暖珊瑚光' }, coverPromptSeed: COVER_SEED, contentPromptSeed: CONTENT_SEED, chapterBeats: ['遇到分享难题', '听见朋友的心情', '一起寻找解决办法', '体会分享后的快乐'] },
  { id: 'tpl_emotion_forest', name: '森林里的心情天气', cover: 'https://images.unsplash.com/photo-1518156677180-95a2893f3e9f?w=500&q=80', ageGroup: '3-6岁', theme: '情绪管理', educationalGoal: '识别情绪', scene: '静谧森林', mainCharacter: { name: '芽芽', role: '会观察心情的小鹿', personality: '细腻好奇，愿意慢慢表达' }, duration: 'short', description: '小鹿把心情比作天气，在森林伙伴的陪伴下学会发现、说出并照顾自己的感受。', isRecommended: true, useCount: 0, visualStyle: { ...DEFAULT_STYLE, palette: '雾霾蓝、蜜桃粉、薄荷绿', lighting: '穿过树叶的柔和晨光与情绪色彩' }, coverPromptSeed: COVER_SEED, contentPromptSeed: CONTENT_SEED, chapterBeats: ['察觉心情变化', '给情绪找到名字', '用呼吸和倾诉让心情变轻'] },
  { id: 'tpl_habits_dinosaur', name: '恐龙乐园的滴答任务', cover: 'https://images.unsplash.com/photo-1518156677180-95a2893f3e9f?w=500&q=80', ageGroup: '3-6岁', theme: '习惯养成', educationalGoal: '规律作息', scene: '恐龙乐园', mainCharacter: { name: '滴滴', role: '爱把事情留到最后的小恐龙', personality: '活泼热心，需要一点提醒' }, duration: 'short', description: '小恐龙用游戏化的小任务安排一天，逐渐发现规律作息能让每天都更轻松好玩。', isRecommended: true, useCount: 0, visualStyle: { ...DEFAULT_STYLE, palette: '柠檬黄、青草绿、奶油橙', lighting: '明亮晴朗的日间阳光' }, coverPromptSeed: COVER_SEED, contentPromptSeed: CONTENT_SEED, chapterBeats: ['发现时间被打乱', '把习惯变成闯关任务', '完成一项小习惯', '享受有秩序的一天'] },
  { id: 'tpl_cognition_space', name: '星球上的颜色旅行', cover: 'https://images.unsplash.com/photo-1518156677180-95a2893f3e9f?w=500&q=80', ageGroup: '3-6岁', theme: '认知启蒙', educationalGoal: '颜色认知', scene: '太空星球', mainCharacter: { name: '星星', role: '喜欢观察颜色的小宇航员', personality: '好奇专注，善于提问' }, duration: 'short', description: '跟着小宇航员穿梭彩色星球，在观察、比较和寻找的过程中认识身边的颜色。', isRecommended: true, useCount: 0, visualStyle: { ...DEFAULT_STYLE, palette: '湖水蓝、明黄、珊瑚红', lighting: '星光与彩色星球反射出的清晰柔光' }, coverPromptSeed: COVER_SEED, contentPromptSeed: CONTENT_SEED, chapterBeats: ['发现颜色线索', '比较不同颜色', '把颜色和物品联系起来'] },
];
...
const COMBINATION_THEMES = ['睡前安抚', '勇敢自信', '友情人际', '情绪管理', '习惯养成', '认知启蒙'];
const COMBINATION_SCENES = ['静谧森林', '温馨家庭', '太空星球', '海底世界', '魔法城堡', '恐龙乐园'];
...
function getPrimaryCharacter(input) {
  const first = (input.characters && input.characters[0]) || {};
  return {
    name: first.name || '小宝贝',
...
function buildCombinationTemplate(theme, scene, index) {
  ...
  return {
    id, name: `${scene}的${character.name}${theme}魔法`, cover: 'https://images.unsplash.com/photo-1518156677180-95a2893f3e9f?w=500&q=80',
    ageGroup: theme === '认知启蒙' ? '3-6岁' : '3-7岁', theme, educationalGoal: goal, scene,
    mainCharacter: character, duration,
    description: `在${scene}里，${character.name}遇到一个和"${theme}"有关的小难题。它和伙伴一起探索${sceneProfile.details}，用温柔又有趣的方式学会${goal}。`,
    isRecommended: index === 0, useCount: 0,
    ...
    chapterBeats: [`在${scene}中发现${theme}主题的小线索`, `遇到需要${goal}的具体情节`, `和伙伴一起尝试一个可行办法`, `用行动完成成长并回到温暖的情绪落点`],
  };
}

const COMBINATION = COMBINATION_THEMES.flatMap(theme =>
  COMBINATION_SCENES.map((scene, index) => buildCombinationTemplate(theme, scene, index)),
);

const STORYBOOK_TEMPLATE_CATALOG = [
  ...CURATED,
  ...COMBINATION.filter(item => !CURATED.some(c => c.theme === item.theme && c.scene === item.scene)),
];

function getStorybookTemplate(input) {
  const matched = input.templateId ? STORYBOOK_TEMPLATE_CATALOG.find(i => i.id === input.templateId) : undefined;
  ...
  const compatible = STORYBOOK_TEMPLATE_CATALOG.find(i => i.theme === input.theme && i.scene === input.scene);
  if (compatible) {
    return { ...compatible, educationalGoal: input.educationalGoal || compatible.educationalGoal };
  }
  return {
    id: 'tpl_dynamic', name: `${input.theme}·${input.scene}绘本`, cover: '', ageGroup: `${input.age}岁`,
    theme: input.theme, educationalGoal: input.educationalGoal, scene: input.scene, mainCharacter: character,
    duration: input.duration.startsWith('long_') ? 'long' : input.duration,
    description: `围绕${input.theme}主题，在${input.scene}中帮助孩子${input.educationalGoal}。`,
    isRecommended: false, useCount: 0,
    visualStyle: { ...DEFAULT_STYLE, palette: themeProfile.palette, lighting: '柔和、有层次且突出角色表情的儿童友好光线' },
    coverPromptSeed: COVER_SEED, contentPromptSeed: CONTENT_SEED,
    chapterBeats: [themeProfile.arc, `在${sceneProfile.setting}中遇到具体情节`, `通过行动完成${input.educationalGoal}`, '回到安全温暖的情绪落点'],
  };
}

function buildStoryTextPrompt(input, template, expectedChapters, durationDesc) {
  const scene = getSceneProfile(input.scene);
  const theme = getThemeProfile(input.theme);
  const characters = (input.characters || []).map((char, index) =>
    `角色 #${index + 1}：${char.name || '无名小伙伴'}，身份/角色：${char.role || '小伙伴'}，性格：${char.personality || '活泼可爱'}${char.customDescription ? `，补充描述：${char.customDescription}` : ''}`,
  ).join('\n');
...
function buildCoverImagePrompt(input, story, template) {
  const theme = getThemeProfile(input.theme);
  const scene = getSceneProfile(input.scene);
  const character = getPrimaryCharacter(input);
...
function buildChapterImagePrompt(input, chapter, template) {
  const scene = getSceneProfile(input.scene);
  const character = getPrimaryCharacter(input);

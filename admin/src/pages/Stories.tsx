...
export default function Stories() {
  ...
  const [list, setList] = useState<StoryDoc[]>([]);
  ...
  const load = useCallback(
    async (p = page, kw = keyword) => {
      setLoading(true);
      const res = await callAdmin('stories/list', { page: p, pageSize: 20, keyword: kw || undefined });
      setLoading(false);
      if (res.success) {
        setList(res.list || []);
  ...
  const onDelete = async (id: string) => {
    const res = await callAdmin('stories/delete', { id });
    if (res.success) {
      message.success('已删除');
      load();
  ...
  const columns = [
    ...
    },
    {
      title: '操作',
      key: 'action',
      render: (_: any, r: StoryDoc) => (
        <Popconfirm title="确认删除该故事？" onConfirm={() => r._id && onDelete(r._id)}>
